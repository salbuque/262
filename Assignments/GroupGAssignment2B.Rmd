...
rolling = rlnorm(mean=r_coeff, sd=0.1*r_coeff, nsample)
summary(rolling)

plot(density(rolling), ylab="Distribution of Rolling Coefficients ")
...
for (i in 1:nrow(possible_cars)) {
  power_for[,i] = auto_pwr(A=possible_cars$area[i], m=possible_cars$mass[i], V = V, crolling = rolling)
}
...
power_pmap = pmap(list(A = possible_cars$area, m=possible_cars$mass), auto_pwr, V=V, crolling = rolling)
